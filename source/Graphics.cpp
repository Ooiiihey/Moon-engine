...
/*
//计算平行光光照
RGBa BaseGraphics::CalculateLight(const ParallelLight& PL, RGBa inputColor, const Vec3& targetCam3D, const Vec3& norVec, const MoonMaterial* ptrMtl) {
	RGBa finalColor;
	// 空指针检查
	if (!ptrMtl) {
	...
	//使用camera space下的方向
	Vec3 lightDir = { -dot(PL.direction, GC.Receive_camera->Forward_vec),
								-dot(PL.direction, GC.Receive_camera->Y_vec),
								-dot(PL.direction, GC.Receive_camera->Z_vec),
							};
	lightDir.normalize();

	RGBa lightColor = PL.Color;
	...
	// 漫反射
	float diffuseFactor = (float)dot(norVec, lightDir);
	RGBa diffuse;
...
//常规版本的三角绘制
void BaseGraphics::DrawFlatTopTriangle(bool direct, const Vertex2D& v0, const Vertex2D& v1, const Vertex2D& v2) {
	...
	for (int y = yStart; y < yEnd; ++y) {
		...
		for (int x = xStart; x < xEnd; ++x) {
			...
			if (GC.buffer->CompareDepth_Smaller(x, y, d) && GC.ptrTexture->getPixel(out.UVCoords.u, out.UVCoords.v).a != 0) {
				...
				if (direct) {
					GC.buffer->PutFinalColor(x, y, GC.ptrTexture->getPixel(out.UVCoords.u, out.UVCoords.v));
				}
				else
				{
...
void BaseGraphics::DrawFlatBottomTriangle(bool condition, const Vertex2D& v0, const Vertex2D& v1, const Vertex2D& v2) {
	// 斜率倒数
...
//常规的带纹理绘制三角形
void BaseGraphics::DrawTriangleTexture(const Vertex2D& v0, const Vertex2D& v1, const Vertex2D& v2, MoonTexture* ptrTexture0, MoonMaterial* ptrMtl1) {
	...
	if (pv1->y == pv0->y) {
		...
		DrawFlatTopTriangle(0, *pv0, *pv1, *pv2);
	}
	else if (pv1->y == pv2->y) {
		if (pv1->x > pv2->x) std::swap(pv2, pv1);
		DrawFlatBottomTriangle(0, *pv0, *pv1, *pv2);
	}
	else {
		const double alphaSplit = (pv1->y - pv0->y) / (pv2->y - pv0->y);
		const double depthBuffersList = 1.0 / (1 / pv0->v3D.x + (1/pv2->v3D.x - 1/pv0->v3D.x) * alphaSplit);//此处进行的是逆深度的插值
		const Vertex2D vi = {	pv0->x + (pv2->x - pv0->x) * alphaSplit,
											pv1->y,
											Vec3(depthBuffersList,
													(pv0->v3D.y / pv0->v3D.x + (pv2->v3D.y / pv2->v3D.x - pv0->v3D.y / pv0->v3D.x) * alphaSplit) * depthBuffersList,
													(pv0->v3D.z / pv0->v3D.x + (pv2->v3D.z / pv2->v3D.x - pv0->v3D.z  / pv0->v3D.x) * alphaSplit) * depthBuffersList),
											Vec2(
											(pv0->UVCoords.u /pv0->v3D.x + (pv2->UVCoords.u /pv2->v3D.x - pv0->UVCoords.u  /pv0->v3D.x) * alphaSplit) * depthBuffersList,
											(pv0->UVCoords.v /pv0->v3D.x + (pv2->UVCoords.v /pv2->v3D.x - pv0->UVCoords.v  /pv0->v3D.x) * alphaSplit) * depthBuffersList),
											Vec3((pv0->norVector.x / pv0->v3D.x + (pv2->norVector.x / pv2->v3D.x - pv0->norVector.x / pv0->v3D.x) * alphaSplit) * depthBuffersList,
													(pv0->norVector.y / pv0->v3D.x + (pv2->norVector.y / pv2->v3D.x - pv0->norVector.y / pv0->v3D.x) * alphaSplit) * depthBuffersList,
													(pv0->norVector.z / pv0->v3D.x + (pv2->norVector.z / pv2->v3D.x - pv0->norVector.z / pv0->v3D.x) * alphaSplit) * depthBuffersList)
												};

		if (pv1->x < vi.x) {
			//major right
			DrawFlatBottomTriangle(0, *pv0, *pv1, vi);
			DrawFlatTopTriangle(0, *pv1, vi, *pv2);
		}
		else {
			//major left
			DrawFlatBottomTriangle(0, *pv0, vi, *pv1);
			DrawFlatTopTriangle(0, vi, *pv1, *pv2);
		}
	}

}

void BaseGraphics::DrawTriangleTexture_direct(const Vertex2D& v0, const Vertex2D& v1, const Vertex2D& v2, MoonTexture* ptrTexture0, MoonMaterial* ptrMtl1) {
	...
	if (pv1->y == pv0->y) {
		if (pv1->x < pv0->x) std::swap(pv0, pv1);
		DrawFlatTopTriangle(1, *pv0, *pv1, *pv2);
	}
	else if (pv1->y == pv2->y) {
		if (pv1->x > pv2->x) std::swap(pv2, pv1);
		DrawFlatBottomTriangle(1, *pv0, *pv1, *pv2);
	}
	else {
		const double alphaSplit = (pv1->y - pv0->y) / (pv2->y - pv0->y);
		const double depthBuffersList = 1.0 / (1 / pv0->v3D.x + (1 / pv2->v3D.x - 1 / pv0->v3D.x) * alphaSplit);//此处进行的是逆深度的插值
		...
		if (pv1->x < vi.x) {
			//major right
			DrawFlatBottomTriangle(1, *pv0, *pv1, vi);
			DrawFlatTopTriangle(1, *pv1, vi, *pv2);
		}
		else {
			//major left
			DrawFlatBottomTriangle(1, *pv0, vi, *pv1);
			DrawFlatTopTriangle(1, vi, *pv1, *pv2);
		}
	}
...
void BaseGraphics::DeferredRender_AmbientLight(AmbientLight& AL) {
	...
	for (int y = GC.ChunkHeight_Begin; y < GC.ChunkHeight_End; ++y) {
		for (int x = GC.ChunkWidth_Begin; x < GC.ChunkWidth_End; ++x) {
			...
			RGBa origialPixel = GC.buffer->GetPixelTextureColor(x, y);
			...
			*/
			GC.buffer->AddFinalColor(x, y, pixel);
		}
	...
	for (int y = GC.ChunkHeight_Begin; y < GC.ChunkHeight_End; ++y) {
		for (int x = GC.ChunkWidth_Begin; x < GC.ChunkWidth_End; ++x) {
			...
			RGBa pixel = PL.Illuminate(
				GC.buffer->GetPixelTextureColor(x, y),
				*GC.Receive_camera,
...
//shadowmapping专属
void Graphics_forShadow::DrawTriangleTexture(const Vertex2D& v0, const Vertex2D& v1, const Vertex2D& v2, MoonTexture* ptrTexture0, MoonMaterial* ptrMtl1) {
